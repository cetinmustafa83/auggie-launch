...
if [[ ! -f "$ROOT/auggie_launch/__init__.py" ]]; then
  echo "error: auggie_launch package not found in $ROOT" >&2
  exit 1
fi
...
if ! python3 -m compileall -q "$ROOT/auggie_launch" >/dev/null; then
  echo "error: the auggie_launch package does not compile; refusing to install" >&2
  exit 1
fi
...
exec env PYTHONPATH="$ROOT\${PYTHONPATH:+:\$PYTHONPATH}" python3 -m auggie_launch "\$@"
EOF
  chmod +x "$WRAPPER"
